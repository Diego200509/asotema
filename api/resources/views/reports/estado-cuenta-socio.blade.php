...
<head>
    ...
    <title>Estado de Cuenta - {{ $socio->nombres }} {{ $socio->apellidos }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="socio-info">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                ...
                <span class="info-value">{{ \Carbon\Carbon::parse($socio->fecha_ingreso)->format('d/m/Y') }}</span>
            </div>
            <div class="info-item">
    ...
    <div class="summary">
        ...
        <div class="summary-grid">
            <div class="summary-item">
                ...
                <div class="summary-value">{{ $estadoCuenta->resumen->total_movimientos ?? 0 }}</div>
            </div>
            <div class="summary-item">
                <div class="summary-label">Total Debe</div>
                <div class="summary-value amount negative">${{ number_format($estadoCuenta->resumen->total_debe ?? 0, 2, '.', ',') }}</div>
            </div>
            <div class="summary-item">
                <div class="summary-label">Total Haber</div>
                <div class="summary-value amount positive">${{ number_format($estadoCuenta->resumen->total_haber ?? 0, 2, '.', ',') }}</div>
            </div>
        </div>
    </div>
    ...
    <table class="movements-table">
        ...
        <tbody>
            @if(isset($estadoCuenta->movimientos) && count($estadoCuenta->movimientos) > 0)
                @foreach($estadoCuenta->movimientos as $movimiento)
                    <tr>
                        ...
                        <td class="amount">${{ number_format($movimiento->saldo_acumulado ?? 0, 2, '.', ',') }}</td>
                    </tr>
                @endforeach
            @else
                <tr>
