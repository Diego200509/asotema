...
class RetiroAhorroRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            // Verificar que el socio tenga saldo suficiente
            if ($this->has('socio_id') && $this->has('monto')) {
                $ahorroService = app(\App\Services\AhorroService::class);
                $saldoActual = $ahorroService->saldoAhorro($this->socio_id);
                
                if ($saldoActual < $this->monto) {
                    $validator->errors()->add('monto', "El socio no tiene saldo suficiente. Saldo actual: $" . number_format($saldoActual, 2));
                }
            }
        });
    }
